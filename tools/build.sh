...
# это защита от публикации фамилий, а не подсказка про опечатку в пути.
if [ "$NAMED" = 1 ]; then
    # В именной версии нет ни шифрования, ни маскировки - там полные фамилии.
    ...
    case "$NAMED_DIR_REAL/" in
        "$REPO"/*|"$REPO"/)
            die "ОСТАНОВЛЕНО: именная версия писалась бы в репозиторий ($NAMED_DIR_REAL)." \
                "В файле полные фамилии - его место только на Диске." ;;
    esac
    case "$NAMED_DIR_REAL" in
...
if [ "$NAMED" = 1 ]; then
    ...
    printf 'verify не запускается: маскировки в именной версии нет по замыслу.\n'
    exit 0
fi
...
MASK=1 PASSWORD="$PASSWORD" AS_OF="$DATE" STAMP="$STAMP" SRC="$SRC" OUT="$REPO/index.html" \
    "$PY" tools/build_report.py

...
ловится глазами по итогам и проверкой имён, а утечка персональных данных -
проверками ниже. Имена детей на странице полные (решение родкома от 27.08.2026,
см. README), поэтому проверяется не форма имени, а совпадение с журналом.
...
# Сокращённая форма имени - «Анна Е.»: имя, пробел, одна заглавная буква с точкой.
# У детей на странице с 27.08.2026 стоят полные имена из журнала, и проверять их
# этим шаблоном больше нечего - вместо формы имени сверяется само содержимое,
# см. names_check(). Казначей остался подписан коротко, эта проверка про него.
...
# не годится - так называлась прежняя вкладка, а ключ D.spends живёт в скрипте
# страницы; проверка ловила бы себя.
...
def names_check(data, spisok):
    """Имена на странице - это имена из журнала, слово в слово.

    Раньше здесь стоял шаблон «Артур Е.»: он ловил сборку без маскировки. Маски
    больше нет, и на её месте осмысленна обратная проверка - что имя не потерялось
    и не съехало по дороге. Она ловит и перепутанные строки (имя одного ребёнка
    рядом с долгом другого), и обрезанное на полпути имя, чего шаблон формы не
    видел в принципе.

    Списки участников внутри сборов проверяются тем же набором: имя там берётся
    из того же поля, но лежит в другой ветке payload, и разъехаться они могут.
    """
    uch, first, last = spisok
    want = {str(uch.cell(row=r, column=3).value or "").strip()
            for r in range(first, last + 1)
            if str(uch.cell(row=r, column=3).value or "").strip()
            and not str(uch.cell(row=r, column=12).value or "").strip().lower().startswith("не ученик")}
    got = {k["name"] for k in data["kids"] if not k.get("ext")}
    check(want == got, f"имена детей на странице совпадают с журналом ({len(got)} шт.)",
          f"разошлись: на странице лишние {sorted(got - want) or '-'}, "
          f"в журнале не нашлись {sorted(want - got) or '-'}")
    # Технические строки («Соседи», учитель) в сборах не участвуют, но проверка
    # не должна падать, если однажды начнут: их имена берутся из того же места.
    tech = {k["name"] for k in data["kids"] if k.get("ext")}
    parts = {p["name"] for s in data["sbory"] for p in s.get("parts", [])} - tech
    check(parts <= want, "имена участников в сборах те же, что в списке класса",
          f"в сборах есть имена, которых нет в списке: {sorted(parts - want)}")
...
def plural(n, one, few, many):
    ...
    a, b = n % 10, n % 100
    return many if 11 <= b <= 14 else (one if a == 1 else (few if 2 <= a <= 4 else many))
...
def main():
    ...
    # Лента кассы. Листа «Движение» может не быть - у прошлогоднего журнала его нет.
    flow = data.get("flow", [])
    ...
    if flow:
        print(f"        движение: {len(flow)} "
              f"{plural(len(flow), 'операция', 'операции', 'операций')}, "
              f"последний остаток {rub(flow[-1]['bal'])} ₽")
    ...
    # Личные списания: часть уже потраченного, адресованная конкретным детям.
    # В «потрачено» они уже входят - лист их не добавляет, а делит иначе, - поэтому
    # поровну делится не всё «потрачено», а остаток за вычетом адресных трат.
    personal = [(k, o) for k in kids for o in k.get("own", [])]
    pers_total = sum(o["amount"] for _, o in personal)
    if personal:
    ...
    if flow:
        # Вся ценность ленты в том, что по ней можно пройти сверху вниз и прийти
        # к остатку из шапки. Значит проверяется и каждый шаг, и итог.
        prev, bad = 0, []
        for f in flow:
            want = prev + f.get("in", 0) - f.get("out", 0)
            if abs(want - f.get("bal", 0)) > 0.005:
                bad.append(f"{f.get('date', '')} «{f.get('what', '')}»")
            prev = f.get("bal", 0)
        check(not bad, f"остаток в ленте сходится на каждом шаге ({len(flow)} "
                       f"{plural(len(flow), 'операция', 'операции', 'операций')})",
              f"остаток разъехался на {len(bad)} строках, первая - {bad[0] if bad else ''}")
        check(abs(flow[-1].get("bal", 0) - t["rest"]) < 0.005,
              f"последний остаток ленты равен остатку кассы ({rub(t['rest'])} ₽)",
              f"лента заканчивается на {rub(flow[-1].get('bal', 0))}, "
              f"а в итогах {rub(t['rest'])}")
        # Возмещение стоит в ленте приходом, а в «Расходах» - минусом, поэтому
        # сумма прихода со «собрано» не сходится и сходиться не должна. Сходится
        # именно разность: столько денег и осталось на карте.
        got = sum(f.get("in", 0) for f in flow) - sum(f.get("out", 0) for f in flow)
        check(abs(got - t["rest"]) < 0.005,
              "пришло минус ушло по всей ленте = остаток кассы",
    ...
    if personal:
        ...
        stray = sorted({k["name"] for k, _ in personal if k.get("ext")})
        check(not stray, "личные списания адресованы только ученикам класса",
              f"списание адресовано не ученику: {', '.join(stray)}")
        # Итог по ребёнку обязан сходиться со своими же строками - иначе в карточке
        # расшифровка не сложится в ту сумму, что стоит над ней.
        bad = [k["name"] for k in kids
               if abs(sum(o["amount"] for o in k.get("own", []))
                      - k.get("ownSum", 0)) > 0.005]
        check(not bad, "личные списания сходятся с итогом по каждому ребёнку",
              f"расходятся у: {', '.join(bad)}")
